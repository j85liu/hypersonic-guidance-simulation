...
for k = 1:N
    ...
    if gps_available
        ...
        % Fix dimension mismatch: ensure all terms are row vectors
        ins_noise_variance = total_ins_error.^2;  % 3x1 vector
        vel_noise_variance = 2^2 * ones(1,3);    % 1x3 vector
        R = diag([ins_noise_variance(:)', vel_noise_variance]);  % Convert to row vectors
    end
